...
class BleHogpService : Service() {
    ...
    override fun onDestroy() {
        ...
        if (hasPermissions) {
            try { advertiser?.stopAdvertising(adCb) } catch (e: SecurityException) {
    ...
    private fun setupGattServices() {
        ...
        kbInputChar = BluetoothGattCharacteristic(
            ...
            it.addDescriptor(BluetoothGattDescriptor(UUID_CCC,
        ...
        kbInputReportChar = BluetoothGattCharacteristic(
            ...
            it.addDescriptor(BluetoothGattDescriptor(UUID_REPORT_REF,
                BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED).also { d -> d.setValue(byteArrayOf(0x01, 0x01)) })
            it.addDescriptor(BluetoothGattDescriptor(UUID_CCC,
                BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED or BluetoothGattDescriptor.PERMISSION_WRITE_ENCRYPTED))
        }
        ...
        hidService.addCharacteristic(BluetoothGattCharacteristic(
            UUID_BOOT_KB_OUTPUT,
        ...
        mouseInputReportChar = BluetoothGattCharacteristic(
            ...
            it.addDescriptor(BluetoothGattDescriptor(UUID_REPORT_REF,
                BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED).also { d -> d.setValue(byteArrayOf(0x02, 0x01)) })
            it.addDescriptor(BluetoothGattDescriptor(UUID_CCC,
                BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED or BluetoothGattDescriptor.PERMISSION_WRITE_ENCRYPTED))
        }
    ...
    private fun startAdvertising(adapter: BluetoothAdapter) {
        ...
        if (hasPermissions) {
            try { advertiser?.startAdvertising(settings, data, adCb) } catch (e: SecurityException) {
    ...
    private val gattCb = object : BluetoothGattServerCallback() {
        ...
        override fun onCharacteristicReadRequest(
            device: BluetoothDevice, requestId: Int, offset: Int, char: BluetoothGattCharacteristic
        ) {
        ...
        override fun onCharacteristicWriteRequest(
            device: BluetoothDevice, requestId: Int, char: BluetoothGattCharacteristic,
    ...
    private fun gattSendResponse(device: BluetoothDevice, requestId: Int, status: Int, offset: Int, value: ByteArray?) {
        val hasConnect = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED
        } else true
        if (!hasConnect) {
