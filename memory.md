## 2026-06-10T20:59:01Z - Claude Opus 4.8 - BlueDeck Release Candidate Fix 3 COMPLETE (all 8 phases)

Spec/TODO: docs/BLUEDECK_RELEASE_CANDIDATE_FIX3_{SPEC,TODO}.md (note: "Release Candidate" Fix 3, a hardening pass — NOT the older Haiku "FIX3" physical-HID address-model work below). Review→responses3.md→replies3.md→Ralph loop, one commit per phase (7da7b85..9a4a8ff).

- P1 Permission callback semantics: RequestMultiplePermissions callback maps are PARTIAL (only that round's perms). Old code checked the callback map against the full required list → a persisted-BLE startup where CONNECT was already granted and only ADVERTISE was requested falsely fell back to Classic. Fix: new pure StartupPermissionDecision.decide(plan, grantedFullState) + allGranted(), and thin Android PermissionGrantChecker (hasAll/missing/grantedSet via ContextCompat.checkSelfPermission). MainActivity.onStartupPermissionResult() and SettingsScreen BLE launcher now re-read actual OS state. New StartupPermissionDecisionTest (9 cases).
- P2 BluetoothService receiver: registerReceiver(...,Context.RECEIVER_NOT_EXPORTED) was the API-33 int-flags overload, UNGUARDED → NoSuchMethodError on minSdk 26. Switched both receivers to ContextCompat.registerReceiver(...,RECEIVER_NOT_EXPORTED), dropped the RECEIVER_EXPORTED try/catch fallback, removed unused SDK_INT_TIRAMISU.
- P3 BleHogpService.onCreate now checks BOTH connect AND advertise (was connect-only) and stopSelf() before BLE setup. Kept checks INLINE (not via helper) so lint MissingPermission data-flow still sees the connect guard for adapter.name/openGattServer (helper-only check made lint error).
- P4 Physical HID docs/tests: ConnectProfile(HID) now PRIMARY on Linux/BlueZ; bluetoothctl connect demoted to fallback/diagnostic (PHYSICAL_HID_TESTING.md Step 4, README already did; BluetoothHidSendReportTest header/logs/skip/timeout messages now emit the dbus-send command, phone addr in underscore object-path form).
- P5 Branding: SDP metadata → "BlueDeck Keyboard/Mouse"/"BlueDeck Android HID"/"BlueDeck" (was Gemini); BleHogpService adapter.name "Bluetooth Keyboard"→"BlueDeck" (B3, may need host re-pair — noted in README); rootProject.name "Bluetooth Keyboard Mouse"→"BlueDeck" (no applicationId/namespace/package change).
- P6 Validation wording: Fix1 Task 12.5 AND Fix2 Task 7.6 manual-smoke checklists had all boxes [x] while their own Validation-results sections said "Pending manual UX smoke" — contradiction. Relabeled with PASS(manual/unit/physical)/PENDING; PENDING items now unchecked. memory.md was already honest.
- P7/P8 Validation (SM-A546E; this Linux box IS the BlueZ host E8:FB:1C:25:E4:C2, phone 8C:6A:3B:5E:D3:48 bonded): clean unit + assemble + lint + ktlint + detekt green; instrumented 97/0 (13 physical skipped); physical HID 13/13 via host-initiated ConnectProfile(HID) fired by me from this host. Keyguard flake: UiComposeInstrumentedTest.scanButtonShowsMessage fails behind a SECURE lock (dismiss-keyguard can't bypass a PIN) — needs human unlock, then passes.

Pending manual UX smoke (carried over, NOT done by Fix 3): fresh-install permission dialogs, persisted-BLE startup, notification-prompt sequencing, on-device reboot boot behavior, startForeground-failure simulation. Not yet pushed at time of this entry.
...
## 2026-06-10T08:01:08Z - Claude Opus 4.8 - Eliminated all detekt MagicNumber findings via named constants
- Starting state: detekt MagicNumber active with `ignorePropertyDeclaration:true`; ~90 `[MagicNumber]` findings across 24 files in app/src/main (these were NOT baselined this round — goal was to fix them, not baseline).
- Approach: extracted each flagged literal VERBATIM into a named constant (top-level `private const val` for Int/Long/Float; would use `private val` for dp/sp but none of the flagged sites were raw dp/sp — they were SDK ints, bit masks, float thresholds, timeouts, page/grid counts). Referenced the constant at the original site. No value/behavior changes.
- Key constant names introduced: SDK_INT_* (OREO=26, MARSHMALLOW=23, S=31, TIRAMISU=33, NOUGAT=24) across BootReceiver/PairingScreen/HidQuickTileService/ServiceNotifications/BleHogpService/BluetoothService/MainScreen/MainActivity/PermissionPolicy/SettingsScreen; MODIFIER_CTRL/SHIFT/ALT/GUI (0x01/0x02/0x04/0x08) in Middleware; LED_CAPS_LOCK=0x02 / LED_SCROLL_LOCK=0x04 in MainActivity (onLeds bit test); MOUSE_BUTTON_MIDDLE=0x04 in BleHogpKeySender; MAX_ROLLOVER_KEYS=6, MODIFIER_BYTE_MASK=0xFF, KEY_PRESS_HOLD_MS in HidReportSender/BleHogpKeySender; TAP_TIMEOUT_MS=220, MOUSE_MOVE_MIN/MAX=-20/20, SCROLL_STEP_BASE_PX=24f, MIN_SCROLL_SPEED=0.1f, THREE_FINGER_TAP=3 in MouseScreen + GestureLogic; COLS_PER_PAGE=3 / MAX_PAGE_INDEX=1 / ARROW_WIDTH_DP=36f / COLUMN_GAP_DP=6f / KEY_CODE_BYTE_MASK=0xFF in the three key-grid screens; KEY_FONT_MIN_SP=10f/MAX_SP=16f in KeyboardScreen; SENSITIVITY/SCROLL_SPEED min=0.5f max=3.0f + KEY_REPEAT_MIN_MS=150/MAX_MS=1000 in SettingsScreen; DEFAULT_RECONNECT_BASE_MS=2000L/MAX_RECONNECT_DELAY_MS=30_000L in ReconnectLogic + BluetoothService; FOREGROUND_START_GUARD_MS=5000L; BINARY_SEARCH_STEP_SP=0.5f in ResponsiveText; FUNCTION_KEY_MIN/MAX=1/12 + FUNCTION_KEY_USAGE_BASE=0x39 in ExtendedKeyMappings; HID_INFO_BCD_HID_LSB=0x11 in BleHogpService.
- DID NOT touch the 3 byte/keycode-table files (@file:Suppress("MagicNumber")): HidDescriptorVariants, HidImeMapper, HidReportBuilder. DID NOT edit detekt.yml / detekt-baseline.xml / .editorconfig.
- Gotcha: `Thread.sleep(10)`/`delay(40)` literals must become `10L`/`40L` const (Long param), not Int. Also ktlintFormat reflows lines after extraction, which can MOVE a previously-unflagged literal into a newly-flagged column — `(0x39 + n)` in ExtendedKeyMappings only got flagged AFTER the first format pass; re-ran detekt after each format until zero. Two manual line-length fixes needed post-format (MouseScreen stepPx, ServiceNotifications pendingFlags).
- Names for review (bit masks): LED_CAPS_LOCK=0x02 / LED_SCROLL_LOCK=0x04 (MainActivity onLeds) and MODIFIER_*/MOUSE_BUTTON_MIDDLE — values copied verbatim, meanings inferred from usage and consistent with existing code comments; HID_INFO_BCD_HID_LSB=0x11 (first byte of HID Information char [0x11,0x01,0x00,0x02]).
- Verified ALL GREEN: detekt (0 MagicNumber), ktlintCheck, compileDebugKotlin, testDebugUnitTest. Did NOT run instrumented tests; did NOT commit.

## 2026-06-10T08:36:07Z - Claude Opus 4.8 - Enforce ReturnCount, SwallowedException, TooGenericExceptionCaught
- All three detekt rules enabled (were active:false).
- **ReturnCount (3 findings → 0, no suppress):** detekt.yml ReturnCount active:true + excludeGuardClauses:true (clears scheduleReconnect/connectLast guard-clause returns). charToHid (8 returns) REFACTORED to a single `return when {...}` expression — initially @Suppress'd it, but per user preference removed the suppress and fixed it properly. Covered by HidImeMapperTest.
- **SwallowedException (4 → 0):** added DebugLog.e logging of the caught exception in BootReceiver (startOnBoot read), BluetoothService (registerReceiver NOT_EXPORTED fallback), MainActivity (2 permission-dialog fallback catches). Real fix.
- **TooGenericExceptionCaught (21 occ / 8 signatures): enabled + BASELINED.** These are defensive `catch (e: Exception) { DebugLog.e(...e.message) }` around Bluetooth/Android calls; each already logs. Narrowing to specific types is UNSAFE (other exception types would propagate and crash the service), so they're grandfathered and the rule enforces new ones. detekt baseline 47 → 55.
- Verified: detekt, ktlintCheck, compileDebugKotlin, testDebugUnitTest, lintDebug green. (ReturnCount/SwallowedException have NO suppress/baseline — genuinely fixed.)

## 2026-06-10T08:50:55Z - Claude Opus 4.8 - Tighten BleHogpService @Suppress("DEPRECATION") (method-level → shims)
- Replaced the 4 broad method/object-level @Suppress("DEPRECATION") (setupGattServices, gattCb, notifyKeyboard, notifyMouse) with 4 tightly-scoped private compat-shim extensions, each wrapping ONE deprecated API: BluetoothGattCharacteristic.setValueCompat/valueCompat and BluetoothGattDescriptor.setValueCompat/valueCompat (setValue()/.value deprecated API 33 but required for GATT server + minSdk 26). Call sites now use the *Compat shims (no suppress).
...
## 2026-06-10T09:46:08Z - Claude Opus 4.8 - Fix UnusedParameter + FunctionOnlyReturningConstant (ForegroundServiceLogic)
- buildNotificationText(connectedName) ignored its param and returned a constant ("Tap to manage connection") → both findings. Made it use the param: returns "Connected to $connectedName" when present, else "Tap to manage connection". Fixes both (param used + varied return). Updated ForegroundServiceLogicTest to assert null/blank/connected branches. Dropped both baseline entries (51→49).
- Note: it's a pure helper in the ForegroundServiceLogic test-helper object (called only by its test, not wired to the live notification — ServiceNotifications still hardcodes text). No production behavior change. Wiring it into the live FGS notification (dynamic "Connected to X") is a possible future enhancement.
- Verified: detekt, ktlint, compile, unit, lint all green. detekt baseline now 49 — entirely structural BT-service complexity (CyclomaticComplexMethod 15, LongMethod 14, TooManyFunctions 7, NestedBlockDepth 5) + 8 defensive TooGenericExceptionCaught.

## 2026-06-10T10:04:01Z - Claude Opus 4.8 - Resolve all TooGenericExceptionCaught (helper + documented suppress)
- User chose the runCatchingLogged-helper approach. Added `inline fun runCatchingLogged(tag, message, block)` to DebugLog.kt (one documented @Suppress("TooGenericExceptionCaught") on the helper's broad catch).
- Converted the 5 pure log-and-continue Unit catches to runCatchingLogged: MainActivity bind classic/BLE (serviceBound/bleHogpBound = bindService(...) preserved inside block), BluetoothService post-settings-notification, send-missing-perm-broadcast, unpair-reflection. Behavior identical.
...
## 2026-06-10T12:04:53Z - Claude Opus 4.8 - UNIT_TESTS1 Ralph loop COMPLETE (UT-01..10)
- Ran docs/UNIT_TESTS1_TODO.md as a Ralph loop; one task per commit, 5-gate verified each.
- Phase 1 (BleHogpKeySender, the untested twin of BluetoothKeySender): UT-01 added HogpNotifier seam (BleHogpService implements it; BleHogpKeySender depends on the interface). UT-02..05 added BleHogpKeySenderTest with a FakeHogpNotifier asserting exact report bytes — keyboard (modifier/dedupe/6-key cap/keyup), mouse (move+clamp, button mask, click press->release), caps/scroll-lock sequences, and BLE no-op commands.
- Phase 2 (HidReportSender host coverage): UT-06 extracted a HidReportTransport seam — HidReportSender now owns only the report-building state machine; platform delivery (device/HID/permission/sendReport + keyboard-vs-mouse error handling) moved verbatim into a new BluetoothHidTransport class (kept BluetoothService from growing -> avoided a LargeClass finding). UT-07/08 added HidReportSenderTest (kbd modifier/rollover-evict/keypress, mouse move/clamp/buttons/clicks, FULL descriptor + scroll, SIMPLE scroll no-op).
